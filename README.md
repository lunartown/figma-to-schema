...
각 테이블은 독립된 **Frame**으로 작성하며, 다음 구조를 따릅니다:

**테이블 구조:**
- **헤더 행**: 회색 배경 (#d9d9d9) + 컬럼명 (Field, Type, Required, Description)
- **데이터 행**: 노란색 배경 (#F5FBB5) + 필드 정보

**필수 컬럼:**
- `Required`: 필수 여부 (● 또는 빈 값)
- `Field`: 필드명
- `Type`: 타입 (string, number, boolean, object, array 등)
- `Description`: 설명 (선택)
...
#### 2. 계층 구조 표현 (object/array 타입)

object나 array 타입의 필드는 **별도 테이블**로 오른쪽에 작성합니다.
...
├─────────────────────┤        ├──────────────────────┤
│ Req │ Field │ Type  │        │ Req │ Field │ Type   │
├─────────────────────┤        ├──────────────────────┤
│ ●   │ id    │ number│        │ ●   │ age   │ number │
│ ●   │ name  │ string│        │     │ bio   │ string │
│     │profile│ object│───────>│ ●   │ avatar│ string │
│     │ tags  │ array │─┐      └──────────────────────┘
└─────────────────────┘ │
                        │      ┌──────────────────────┐
                        │      │ tags (array item)    │
...
**규칙:**
1. 부모 테이블에서 `object` 또는 `array` 타입의 필드가 있으면
2. 해당 필드의 상세 구조를 오른쪽에 **새 테이블**로 작성
3. 자식 테이블은 부모 테이블과 **수직으로 정렬** (y 좌표)
4. array 타입의 경우 자식 테이블 제목에 "(array item)" 표시
...
1. **색상 기반 인식**
   - 헤더: #d9d9d9 (회색) 배경을 가진 노드를 컬럼 헤더로 인식
   - 데이터: #F5FBB5 (노란색) 배경을 가진 노드를 데이터 행으로 인식

2. **계층 구조 추론**
   - x 좌표: 왼쪽 테이블이 부모, 오른쪽이 자식
   - y 좌표: 수직으로 정렬된 테이블끼리 연결
   - 부모 테이블의 object/array 필드와 매칭

3. **엔드포인트 인식**
   - 테이블 제목에서 `[METHOD] /path` 패턴 추출
   - 엔드포인트가 있는 테이블을 루트로 간주
