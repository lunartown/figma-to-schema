 ...
 * JSON Schema를 MySQL DDL로 변환
 */
export function generateMySQLFromSchema(
  schema: JsonSchema,
  options?: {
    tableName?: string;
    ...
    for (const [fieldName, fieldSchema] of Object.entries(schema.properties)) {
      ...
      if (fieldSchema.type === 'object' && fieldSchema.properties) {
        ...
        columnDefs.push({
          ...
          default: '',
          comment: includeComments && fieldSchema.description ? `COMMENT '${escapeComment(fieldSchema.description)}'` : ''
        });
        ...
        // 일반 컬럼
        const sqlType = jsonTypeToMySQLType(fieldSchema);
...
/**
 * JSON Schema 타입을 MySQL 타입으로 변환
 */
function jsonTypeToMySQLType(schema: JsonSchema): string {
  ...
  switch (type) {
    case 'string':
      ...
      } else if (schema.maxLength && schema.maxLength <= 65535) {
        return 'TEXT';
      } else {
        return 'VARCHAR(255)';
      }
