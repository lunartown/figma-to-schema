 ...
 * JSON Schema를 Oracle DDL로 변환
 */
export function generateOracleFromSchema(
  schema: JsonSchema,
  options?: {
    tableName?: string;
  ...
  if (schema.type === 'object' && schema.properties) {
    ...
    for (const [fieldName, fieldSchema] of Object.entries(schema.properties)) {
      ...
      if (fieldSchema.type === 'object' && fieldSchema.properties) {
        ...
        // 일반 컬럼
        const sqlType = jsonTypeToOracleType(fieldSchema);
...
/**
 * JSON Schema 타입을 Oracle 타입으로 변환
 */
function jsonTypeToOracleType(schema: JsonSchema): string {
  ...
  switch (type) {
    case 'string':
      ...
      } else if (schema.maxLength && schema.maxLength <= 4000) {
        return `VARCHAR2(${schema.maxLength})`;
      } else {
        return 'CLOB';
      }
